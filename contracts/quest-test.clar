...
(define-public (join-quest (quest-id (string-ascii 36)) (participation-amount uint) (use-token <token>))
  (let (
    ...
    (participant-key { quest-id: quest-id, participant: tx-sender })
      ...
      (try! (contract-call? use-token transfer participation-amount tx-sender (as-contract tx-sender) none))
    ...
    (is-completed (is-eq new-count ACTIVITIES_PER_QUEST))
      ...
      (if is-completed
        (begin
          ...
          (as-contract (try! (contract-call? use-token transfer (get locked-amount participant) tx-sender (get participant participant-key) none)))
